<?php
session_start();
include('config/db.php');

// Redirect if not logged in
if (!isset($_SESSION['user_id'])) {
    header("Location: index.php");
    exit();
}

$user_id = $_SESSION['user_id'];
$success = "";
$error = "";

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $recipient_username = trim($_POST['recipient']);
    $amount = floatval($_POST['amount']);
    $entered_pin = $_POST['pin'];

    // Get sender's PIN and balance
    $stmt = $conn->prepare("SELECT pin FROM users WHERE id = ?");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $stmt->bind_result($correct_pin);
    $stmt->fetch();
    $stmt->close();

    if ($entered_pin !== $correct_pin) {
        $error = "Invalid PIN.";
    } elseif ($amount <= 0 || empty($recipient_username)) {
        $error = "Invalid recipient or amount.";
    } else {
        // Check current balance
        $bal_stmt = $conn->prepare("SELECT balance FROM accounts WHERE user_id = ?");
        $bal_stmt->bind_param("i", $user_id);
        $bal_stmt->execute();
        $bal_stmt->bind_result($balance);
        $bal_stmt->fetch();
        $bal_stmt->close();

        if ($amount > $balance) {
            $error = "Insufficient funds.";
        } else {
            // Get recipient's user_id
            $rec_stmt = $conn->prepare("SELECT id FROM users WHERE username = ?");
            $rec_stmt->bind_param("s", $recipient_username);
            $rec_stmt->execute();
            $rec_stmt->bind_result($recipient_id);
            if ($rec_stmt->fetch()) {
                $rec_stmt->close();

                // Begin transaction
                $conn->begin_transaction();

                try {
                    // Deduct from sender
                    $deduct = $conn->prepare("UPDATE accounts SET balance = balance - ? WHERE user_id = ?");
                    $deduct->bind_param("di", $amount, $user_id);
                    $deduct->execute();

                    // Add to recipient
                    $add = $conn->prepare("UPDATE accounts SET balance = balance + ? WHERE user_id = ?");
                    $add->bind_param("di", $amount, $recipient_id);
                    $add->execute();

                    $conn->commit();
                    $success = "Successfully transferred ₱" . number_format($amount, 2) . " to " . htmlspecialchars($recipient_username);
                } catch (Exception $e) {
                    $conn->rollback();
                    $error = "Transfer failed. Please try again.";
                }
            } else {
                $error = "Recipient not found.";
            }
        }
    }
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Transfer Funds</title>
    <link rel="stylesheet" href="css/style.css">
</head>
<body>
    <h2>Transfer Funds</h2>

    <?php if ($success): ?>
        <p style="color: green;"><?php echo $success; ?></p>
    <?php elseif ($error): ?>
        <p style="color: red;"><?php echo $error; ?></p>
    <?php endif; ?>

    <form method="POST">
        <label for="recipient">Recipient Username:</label>
        <input type="text" name="recipient" required><br><br>

        <label for="amount">Amount (₱):</label>
        <input type="number" step="0.01" name="amount" required><br><br>

        <label for="pin">Enter PIN:</label>
        <input type="password" name="pin" maxlength="4" required><br><br>

        <input type="submit" value="Transfer">
    </form>

    <br>
    <a href="dashboard.php">Back to Dashboard</a>
</body>
</html>
